...
namespace BattleRpg
{
    class Program
    {
        static void Main()
        {
            ...
            bool canSingleHelpOfSpirits = true;
            int healthHero = 120;
            int healthEnemy = 200;
            ...
            while (healthHero >= pointsBeforeDeathPerson & healthEnemy >= pointsBeforeDeathPerson)
            {
                ...
                switch (userInput)
                {
                    ...
                    case CommandAttackSpirit:
                        if (canSingleHelpOfSpirits == true)
                        {
                            ...
                            healthHero += enemyDamage;
                            canSingleHelpOfSpirits = false;
                        }
                        else if (canSingleHelpOfSpirits == false)
                        {
                    ...
                    case CommandAttackCurse:
                        ...
                        {
                            int shortageMagic = conditionApplicationMagicVoodooPoints - storageMagicPointVoodoo;
                            Console.WriteLine($"\nВы не можете использовать куклу вуду у вас не достаточно {shortageMagic} очков магии вуду");
                        }
                        break;
                }
            }
